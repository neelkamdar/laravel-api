    ...
    <div class="table-section bill-tbl w-100 mt-10">
        <table class="table w-100 mt-10">
            ...
            <tr align="center">
                <td>{{++$no}}</td>
                <td>{{$product->name}}</td>
                <td>{{$currencySymbol}} {{Helpers::currencyConvert($currencyCode, $product->pivot->single_price)}}</td>
                <td>{{$product->pivot->quantity}}</td>
                <td>{{$currencySymbol}} {{Helpers::currencyConvert($currencyCode, $product->pivot->subtotal)}}</td>
                <td>{{$currencySymbol}} {{Helpers::currencyConvert($currencyCode, $product->pivot->shipping_cost)}}</td>
                <td>{{$currencySymbol}} {{Helpers::currencyConvert($currencyCode, ($product->pivot->subtotal + $product->pivot->shipping_cost + $product->pivot->tax))}}</td>
            </tr>
            @endforeach
            <tr>
                <td colspan="7">
                    <div class="total-part">
                        ...
                        <div class="total-right w-15 float-left text-bold" align="right">
                            <p>{{$currencySymbol}} {{Helpers::currencyConvert($currencyCode, $order->amount)}}</p>
                            <p>{{$currencySymbol}} {{Helpers::currencyConvert($currencyCode, $order->tax_total)}}</p>
                            <p>{{$currencySymbol}} {{Helpers::currencyConvert($currencyCode, $order->shipping_total)}}</p>
                            <p>{{$currencySymbol}} {{Helpers::currencyConvert($currencyCode, $order->total)}}</p>
                        </div>
                        <div style="clear: both;"></div>
                    </div>
                </td>
