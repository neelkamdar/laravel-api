...
const ThemeOptionAllTabs = ({ activeTab, values, setFieldValue, errors, touched }) => {
    ...
    const { data: categoryData, isLoading } = useQuery({ queryKey: [Category], queryFn: () => request({ url: Category, params: { status: 1 } },router), refetchOnWindowFocus: false, select: (data) => data.data.data });
    ...
    return (
        <Col xl="7" lg="8">
            <TabContent activeTab={activeTab}>
                ...
                <TabPane tabId="4"><CollectionProduct values={values} setFieldValue={setFieldValue} categoryData={categoryData} /></TabPane>
