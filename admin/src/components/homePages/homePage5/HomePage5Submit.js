...
const HomePage5Submit = (values, mutate) => {
    ...
    if (values['productDealsImage']) {
        values['content']['product_with_deals']['deal_of_days']['image_url'] = values['productDealsImage'].hasOwnProperty('asset_url') ? values['productDealsImage']?.asset_url : values['productDealsImage']?.original_url
    } else values['content']['product_with_deals']['deal_of_days']['image_url'] = ''

    if (values['homeBannerImage']) {
        values['content']['home_banner']['main_banner']['image_url'] = values['homeBannerImage'].hasOwnProperty('asset_url') ? values['homeBannerImage']?.asset_url : values['homeBannerImage']?.original_url
    } else values['content']['home_banner']['main_banner']['image_url'] = ''

    if (values['categoryProductImage']) {
        values['content']['categories_image_list']['image_url'] = values['categoryProductImage'].hasOwnProperty('asset_url') ? values['categoryProductImage']?.asset_url : values['categoryProductImage']?.original_url
    } else values['content']['categories_image_list']['image_url'] = ''

    if (values['fullWidthImage']) {
        values['content']['full_width_banner']['image_url'] = values['fullWidthImage'].hasOwnProperty('asset_url') ? values['fullWidthImage']?.asset_url : values['fullWidthImage']?.original_url
    } else values['content']['full_width_banner']['image_url'] = ''

    if (values['twoColumnBanner1Image']) {
        values['content']['two_column_banners']['banner_1']['image_url'] = values['twoColumnBanner1Image'].hasOwnProperty('asset_url') ? values['twoColumnBanner1Image']?.asset_url : values['twoColumnBanner1Image']?.original_url
    } else values['content']['two_column_banners']['banner_1']['image_url'] = ''

    if (values['twoColumnBanner2Image']) {
        values['content']['two_column_banners']['banner_2']['image_url'] = values['twoColumnBanner2Image'].hasOwnProperty('asset_url') ? values['twoColumnBanner2Image']?.asset_url : values['twoColumnBanner2Image']?.original_url
    } else values['content']['two_column_banners']['banner_2']['image_url'] = ''

    if (values['deliveryBanners1Image']) {
        values['content']['delivery_banners']['banner_1']['image_url'] = values['deliveryBanners1Image'].hasOwnProperty('asset_url') ? values['deliveryBanners1Image']?.asset_url : values['deliveryBanners1Image']?.original_url
    } else values['content']['delivery_banners']['banner_1']['image_url'] = ''

    if (values['deliveryBanners2Image']) {
        values['content']['delivery_banners']['banner_2']['image_url'] = values['deliveryBanners2Image'].hasOwnProperty('asset_url') ? values['deliveryBanners2Image']?.asset_url : values['deliveryBanners2Image']?.original_url
    } else values['content']['delivery_banners']['banner_2']['image_url'] = ''
    ...
    values['content']['bank_wallet_offers']['offers'].forEach((elem, i) => {
        if (values[`bankOfferImage${i}`]) {
            values['content']['bank_wallet_offers']['offers'][i]['image_url'] = values[`bankOfferImage${i}`].hasOwnProperty('asset_url') ? values[`bankOfferImage${i}`]?.asset_url : values[`bankOfferImage${i}`]?.original_url
        } else values['content']['bank_wallet_offers']['offers'][i]['image_url'] = ""
    })

    values['content']['bank_wallet_offers']['offers'].forEach((elem, i) => {
        if (values[`bankOfferBankImage${i}`]) {
            values['content']['bank_wallet_offers']['offers'][i]['bank_image_url'] = values[`bankOfferBankImage${i}`].hasOwnProperty('asset_url') ? values[`bankOfferBankImage${i}`]?.asset_url : values[`bankOfferBankImage${i}`]?.original_url
