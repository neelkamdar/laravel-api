...
class OrderRepository extends BaseRepository
{
        ...
        try {
            ...
            if ($order) {
                if ($this->verifySingleOrder($order)) {
                    return $this->verifyPayment($order)->toArray();
                }
    ...
    public function updateOrderStatusActivities($order, $reqStatus, $changed_at = null)
    {
        $sequence = $this->orderStatus->getSequenceByName($reqStatus);
        ...
        if ($order?->is_digital_only) {
            ...
            $order_sequences = collect(range(1, $sequence))?->reject(fn($item) => ($sequence > $cancelSequence && in_array($item, $excludeSequences)))->values()->all();
        } else {
        ...
        if ($order_sequences && is_array($order_sequences)) {
            foreach ($order_sequences as $order_sequence) {
                ...
                if ($status) {
                    ...
                    $orderStatusActivity = $order->order_status_activities()?->where('status', $status)?->first();
                    if($orderStatusActivity) {
                        if($orderStatusActivity->status == $reqStatus) {
                            $orderStatusActivity?->update([
                            'status' => $status,
                            'changed_at' => $changed_at
                            ]);
                        }
                    } else {
                        $order->order_status_activities()?->create([
                            'status' => $status,
                            'changed_at' => $changed_at
                        ]);
                    }
