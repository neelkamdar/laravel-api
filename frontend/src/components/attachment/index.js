...
const AttachmentContain = ({ isattachment }) => {
    ...
    const { data: attachmentsData, refetch, isLoading, fetchStatus } = useQuery({ queryKey: [attachment], queryFn: () => request({ url: attachment, params: { search, sort: sorting, paginate: paginate, page } }, router), refetchOnWindowFocus: false, enabled: false, select: (data) => data?.data });
    useEffect(() => {
        refetch();
