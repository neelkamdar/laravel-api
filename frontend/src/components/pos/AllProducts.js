...
const AllProducts = ({ setFieldValue, values, dispatch, setCartData, CategoryData, getCategoryId, setStateProduct, setGetCategoryId }) => {
  ...
  // Calling Product API
  const { data, refetch, fetchStatus } = useQuery({ queryKey: [product, getCategoryId], queryFn: () => request({ url: product, params: { category_ids: getCategoryId || productParams.category_ids.join(','), search: productParams?.search, status: 1, paginate: 20, page } },router), refetchOnWindowFocus: false, select: (data) => data.data });
