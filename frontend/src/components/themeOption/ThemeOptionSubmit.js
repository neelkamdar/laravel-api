...
const ThemeOptionSubmit = (values, mutate) => {
    ...
    if (values["collectionCategories"]) {
        values['options']['collection']["collection_categories_ids"] = values["collectionCategories"]
    }
    ...
    values['options']['about_us']['clients']['content']?.forEach((elem, i) => {
        if (values[`clientContentImage${i}`]) {
            values['options']['about_us']['clients']['content'][i]['icon'] = values[`clientContentImage${i}`].hasOwnProperty('asset_url') ? values[`clientContentImage${i}`]?.asset_url : values[`clientContentImage${i}`]?.original_url 
