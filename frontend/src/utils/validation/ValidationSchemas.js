...
export const emailSchema = Yup.string().email("invalid_email").required();
...
export const placePointsSchema = Yup.array().required();


export const fileUploadSchema = (typeKey) =>
  Yup.mixed()
    .required(`${typeKey} is required`)
    .test("fileType", "Unsupported file format", (value) => {
      if (!value) return false;
      const supportedFormats = mediaConfig[typeKey]; // Dynamically fetch based on typeKey
      if (typeof value === "object" && value.type) {
        return supportedFormats.includes(value.type);
      }

      // If value is not an object, assume it's an ID (for existing images)
