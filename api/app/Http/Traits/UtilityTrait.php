...
trait UtilityTrait
{
  ...
  public function verifyOrderUserDetails(Order $order, $request)
  {
    if (!$request->email_or_phone) {
      throw new Exception(__('errors.email_phone_required_for_tracking'), 422);
    }
  ...
  public function isActivePaymentMethod($request, $amount = null)
  {
    $method = $request?->payment_method;
    $settings = Helpers::getSettings();
    if ($this->isEnablePaymentMethod($method)) {
      $defaultCurrencyCode = $request?->code ?? Helpers::getDefaultCurrencyCode();
      if ($method == PaymentMethod::PAYPAL) {
        if (!in_array($defaultCurrencyCode, array_column(PaypalCurrencies::cases(), 'value'))) {
          throw new Exception(__('errors.not_support_method',['defaultCurrencyCode' => $defaultCurrencyCode, 'method' => $method]), 400);
        }
      }

      if ($method == PaymentMethod::PHONEPE) {
        if ($settings['payment_methods'][PaymentMethod::PHONEPE]['sandbox_mode']) {
          if ($defaultCurrencyCode != 'INR') {
          ...
          throw new Exception(__('errors.phonepe_sandbox_limit'), 400);
        }
      }
    ...
    throw new Exception(__('errors.payment_method_disabled'), 400);
  }
  ...
  public function isOutOfStock($products)
  {
    ...
    if (!empty($outOfStockProducts)) {
      throw new Exception(__('errors.product_out_of_stock_or_inactive'), 400);
    }
