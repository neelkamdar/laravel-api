...
const VariationAddToCart = ({ cloneVariation, setVariationModal }) => {
  ...
  return (
    <div className="addtocart_btn">
      <Btn className="btn btn-md fw-bold icon text-white theme-bg-color view-button text-uppercase" disabled={(cloneVariation?.selectedVariation && cloneVariation?.selectedVariation?.stock_status !== "in_stock") || (cloneVariation?.product?.stock_status !== "in_stock" && true)} onClick={() => addToCart(cloneVariation.product)} loading={Number(isLoading)}>
        <RiShoppingCartLine />
