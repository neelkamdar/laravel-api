...
const DashboardChart = () => {
    ...
    const { data, refetch } = useQuery({ queryKey: [DashboardChartAPI], queryFn: () => request({ url: DashboardChartAPI },router), refetchOnWindowFocus: false, enabled: false, select: (data) => data?.data });
    useEffect(() => {
        refetch()
    }, [])
    return (
