...
class ProductRepository extends BaseRepository
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $product = $this->model->create([
                ...
                'is_all_author' => $request->is_all_author,
    ...
    public function update($request, $id)
    {
        ...
        try {
            ...
            if (isset($request['is_all_author'])) {
                $product->authors()->sync([]);
                if (! $request['is_all_author']) {
            ...
            if ($request['is_random_related_products']) {
                if (isset($request['is_all_categories'])) {
                    if($request['is_all_categories']) {
                        $request['categories'] = Category::where('status', true)?->pluck('id')?->toArray() ?? [];
                    }
                }
    ...
    public function relationProductModels($request, $product)
    {
        ...
        if (isset($request->categories) && !$request->is_all_categories) {
            $product->categories()->attach($request->categories);
            $product->categories;
        }

        if (isset($request->authors_id) && !$request->is_all_author) {
            $product->authors()->attach($request->authors_id);
            $product->authors;
        }

        if (isset($request->tags) && !$request->is_all_tags) {
    ...
    public function getProductBySlug($slug)
    {
        try {
            ...
            return $this->model->where('slug', $slug)
                ...
                ->firstOrFail()
                ->setAppends(config('enums.product.appends'))
                ->makeVisible(config('enums.product.visible'))?->toArray();
